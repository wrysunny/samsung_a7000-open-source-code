...
public class DABService extends Service { /*
                                           * * Service Component Identifer
                                           * within the Service (SCIdS) value
                                           */
    private String mScids; /* * Service Identifer (SId) value */
    private String mSid; /* * Ensemble Identifier (EId) value */
    private String mEid; /* * Global Country Code (GCC) value */
    private String mGcc; /*
                          * * X-PAD Applicaton Type (AppTy) and User Applicaton
                          * type value
                          */
    private String mXpad; /* * Packet Address (PA) value */
    private Integer mPa;

    public DABService(String gcc, String eid, String sid, String scids, int pa)
            throws LookupException {
        this(gcc, eid, sid, scids);
        if (pa > 0 && pa < 1023) {
            mPa = pa;
        } else {
            throw new LookupException("Invalid data value. Must be a valid Packet Address integer.");
        }
    }

    public DABService(String gcc, String eid, String sid, String scids, String xpad)
            throws LookupException {
        this(gcc, eid, sid, scids);
        if (xpad.equals(null)) {
    ...
    public String getRadioDNSFqdn() {
        String fqdn = String.format("%s.%s.%s.%s.dab.radiodns.org", mScids, mSid, mEid, mGcc);
        if (mXpad != null || mPa > -1) {
